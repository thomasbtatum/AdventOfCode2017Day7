...
namespace AdventOfCode2017Day7
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string largeInput = @"navfz (187) -> jviwcde, wfwor, vpfabxa
adtfnx (74)
endsg (33)
...
xszevpx (8)";
            ...
            foreach (var line in lines)
            {
                if (line.Contains("->"))
                {
                    ...
                    var children = values.Split(',');
                    foreach (var child in children)
                    {
                        knownChildren.Add(child.Trim());
                    }
                }

            }

            //check for value not present and this is the parent
            foreach (var line in lines)
            {
                if (line.Contains("->"))
                {
                    ...
                    string checkMe = children[0].Trim();
                    if (knownChildren.Add(checkMe))
                    {
                        int x = 1;
                    }
                }
